...
router.get('/:category', function (req, res) {
  ...
  if(category=="books"){
      if(name==undefined || name==""){
          ...
          array=books.filter((books)=>books.name.split(" ").join("").toLowerCase().includes(name.split(" ").join("").toLowerCase()));
      }
  }
      ...
      if(name==undefined){
          ...
          array=movies.filter((movies)=>movies.name.split(" ").join("").toLowerCase().includes(name.split(" ").join("").toLowerCase()));
      }
  }
  if(array.length!==0){
      return res.send({status:"ok",result:array});
  }
